...
let rec expr_mapper ~signed m expr =
  ...
  begin match expr with
    ...
    | [%expr [%hw if [%e? cnd] then [%e? e0] else [%e? e1]]] ->
      `Recurse_Unsigned (app ~signed:`unsigned [%expr mux2 [%e cnd]] e0 e1)
    ...
    | [%expr [%hw.signed if [%e? cnd] then [%e? e0] else [%e? e1]]] ->
      `Recurse_Signed (app ~signed:`signed [%expr mux2 [%e cnd]] e0 e1)
    ...
    | [%expr [%hw [%e? { pexp_desc=Pexp_match(sel, cases); pexp_loc=loc }]]] ->
      let sel = umapper m sel in
      let cases = List.map (fun c -> { c with pc_rhs = umapper m c.pc_rhs }) cases in
      `Return (match_mapper ~loc (resize `unsigned) sel cases)
    (* signed match construct *)
    | [%expr [%hw.signed [%e? { pexp_desc=Pexp_match(sel, cases); pexp_loc=loc }]]] ->
      let sel = smapper m sel in
      let cases = List.map (fun c -> { c with pc_rhs = smapper m c.pc_rhs }) cases in
      `Return (match_mapper ~loc (resize `signed) sel cases)
    (* [%hw ] expression *)
    | [%expr [%hw [%e? e]]] -> `Recurse_Unsigned [%expr [%e umapper m e]]
    | [%expr [%hw.signed [%e? e]]] -> `Recurse_Signed [%expr [%e smapper m e]]
    (* Constant *)
  ...
  | `Recurse_Signed (expr)   -> smapper m expr
  | `Recurse_Unsigned (expr) -> umapper m expr
  | `Return (expr)           -> expr
